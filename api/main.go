...
func main() {
	...
	guessIndex := 0
	codeLength := 4
	code := generateCode(colors, uint8(codeLength))
	...
	for gameState == 2 {
		...
		guess := convertInputToGuess(input, codeLength)
		correntSpots, incorrectSpots := evaluateGuess(append(make([]uint8, 0), guess...), append(make([]uint8, 0), code...))

		guessIndex++
		gameState = determineGameState(correntSpots, uint8(codeLength), uint8(guessLimit), uint8(guessIndex))
		if gameState == 2 {
...
func convertInputToGuess(input string, length int) []uint8 {
